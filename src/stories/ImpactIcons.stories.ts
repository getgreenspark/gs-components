    ...
    template: `
      <div>        
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 24px;">
          <div v-for="icon in icons" :key="icon.name" style="display: flex; flex-direction: column; align-items: center; gap: 8px;">
                      <GsTypography variant="description" style="text-align: center;">{{ icon.name }}</GsTypography>
  
          <div style="width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; background: var(--main-black); border-radius: 8px;">
              <img :src="icon.path" :alt="icon.name" width="24" height="24" />
            </div>
            <GsTypography variant="small-description" style="text-align: center; color: var(--grey-scale-60);">white: /public/icons/impact_types/{{ icon.name.toLowerCase() }}.svg</GsTypography>
            
            <div style="width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; background: var(--main-white); border-radius: 8px; border: 1px solid var(--grey-scale-40);">
              <img :src="icon.blackPath" :alt="icon.name + ' Black'" width="24" height="24" />
            </div>
            <GsTypography variant="small-description" style="text-align: center; color: var(--grey-scale-60);">black: /public/icons/impact_types/{{ icon.name.toLowerCase() }}_black.svg</GsTypography>
